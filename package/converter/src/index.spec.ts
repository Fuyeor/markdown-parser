// @ffm/converter/src/index.spec.ts
// pnpm --filter @ffm/converter test
import { describe, expect, it } from 'vitest';
import { fromHTML } from './index.node';
import fixture from './fixture/spec.json' with { type: 'json' };

describe('converter conversion', () => {
  for (const [section, item] of Object.entries(fixture)) {
    describe(section, () => {
      for (const [index, { desc, text, expect: expected }] of item.entries()) {
        it(desc ?? `case ${index + 1}: ${text.slice(0, 30)}`, () => {
          expect(fromHTML(text)).toBe(expected);
        });
      }
    });
  }
});

describe('converter input validation', () => {
  it('fails fast for non-string input', () => {
    expect(() => fromHTML(null as unknown as string)).toThrow(
      new TypeError('Input must be a string'),
    );
  });
});

describe('converter option', () => {
  it('respects maxConsecutiveBlankLines option for empty paragraphs and blockquotes', () => {
    const html = '<p>First</p><p></p><p></p><p></p><p></p><p>Second</p>';

    // default (keep 1 blank lines)
    expect(fromHTML(html)).toBe('First\n\nSecond');

    // keep 4 blank lines
    expect(fromHTML(html, { maxConsecutiveBlankLines: 4 })).toBe(
      'First\n\n\n\n\nSecond',
    );

    // keep 4 blank lines within blockquote
    const quoteHtml =
      '<blockquote><p>Line1</p><p></p><p></p><p>Line2</p><p>Line3</p></blockquote>';
    expect(fromHTML(quoteHtml, { maxConsecutiveBlankLines: 2 })).toBe(
      '```quote\nLine1\n\n\nLine2\n\nLine3\n```',
    );
  });
});
