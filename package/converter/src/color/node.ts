// @ffm/converter/src/color/node.ts
import { colorNameMap } from './name-map';
...
/** Normalize supported CSS colors into FFM-compatible hexadecimal notation in Node. */
export function parseColorInNode(value: string): string | null {
  const normalized = value.trim().toLowerCase();
  if (!normalized || normalized.includes('var(')) return null;

  const named = colorNameMap[normalized];
...
export function isTransparentColorInNode(value: string): boolean {
  const normalized = value.trim().toLowerCase();
