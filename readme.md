**@ffm/parser**: A lightweight Markdown parser optimized for loading speed and extensibility. It supports CommonMark standards alongside **Fuyeor Flavored Markdown (FFM)** across various frameworks. [**Try the playground.**](https://flavored.fuyeor.com/playground)

> [!IMPORTANT]
> By default, `@ffm/parser` does **not** support bolding with `__`, strikethrough with `~~`, or italics with `_italic_`. Image syntax `![]()` is also disabled by default.
...
```javascript
import { createMarkdownParser, render } from '@ffm/parser';
...
You can also explore it online at [flavored.fuyeor.com](https://flavored.fuyeor.com/playground).
