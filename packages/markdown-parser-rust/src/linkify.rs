...
mod tests {
    ...
    #[test]
    fn links_fuzzy_domains_and_normalizes_protocol() {
        assert_eq!(linkify("Visit fuyeor.com")[0].url, "https://fuyeor.com");
    }

    #[test]
    fn rejects_unknown_fuzzy_tlds() {
        assert!(linkify("example.invalidtld").is_empty());
    }

    #[test]
    fn supports_unicode_russian_tld() {
        let matches = linkify("fuyeor.рф");
        assert_eq!(matches[0].url, "https://fuyeor.рф");
        assert_eq!(matches[0].text, "fuyeor.рф");
    }

    #[test]
    fn matches_the_reference_linkify_fixtures() {
        let cases = [
            ("Visit fuyeor.com.", Some("fuyeor.com")),
            ("Visit fuyeor.com;", Some("fuyeor.com")),
            ("(fuyeor.com)", Some("fuyeor.com")),
            ("Visit fuyeor.cn.", Some("fuyeor.cn")),
            ("Visit fuyeor.co.uk.", Some("fuyeor.co.uk")),
            ("Visit web.dev.", Some("web.dev")),
            ("Visit vercel.app", Some("vercel.app")),
            ("Visit fuyeor.рф.", Some("fuyeor.рф")),
            ("Visit fuyeor.xn--p1ai.", Some("fuyeor.рф")),
            (
                "Profile: https://fuyeor.com/@Fuyeor",
                Some("https://fuyeor.com/@Fuyeor"),
            ),
            (
                "A link with parens: https://en.wikipedia.org/wiki/Link_(disambiguation)",
                Some("https://en.wikipedia.org/wiki/Link_(disambiguation)"),
            ),
            ("Visit fuyeor.123", None),
            ("Visit fuyeor.enterprise", None),
            ("Visit fuyeor.md", None),
            ("Visit fuyeor..com", None),
            ("Visit fuyeor,com", None),
            ("Visit fuyeor.com123", None),
            ("Visit fuyeor.comあ", None),
            ("fuyeor.com💜", Some("fuyeor.com")),
        ];
        for (source, expected) in cases {
            let matches = linkify(source);
            let actual = matches.first().map(|matched| matched.text.as_str());
